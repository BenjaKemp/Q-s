<?php
    print_r($_POST);
  function functionName($currency) {
    $endpoint = 'live';
    $access_key = '1c7910d65922432b4f7ba9117cefedc7';

    //to change source you need an api key on basic plan or higher
    //then add ?source=$currency to the api


    $request = "http://apilayer.net/api/$endpoint?access_key=$access_key";
   ...
   <html lang="en" dir="ltr">
     ...
     <body>
      ...
      if (array_key_exists('currency', $_POST)) {
        ...
        $exchangeRates = functionName($currency);
        foreach ($exchangeRates as $key => $exchangeRate) {
          ?>
          <div>
            <?php echo str_replace($currency, '', $key)?>
